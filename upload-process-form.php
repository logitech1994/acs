...
?>
		<h3><?php _e('Ҳужжат юклаш амалга оширилди','cftp_admin'); ?></h3>
		<table id="uploaded_files_tbl" class="footable" data-page-size="<?php echo FOOTABLE_PAGING_NUMBER; ?>">
			<thead>
				<tr>
					<th data-sort-initial="true"><?php _e('Номи','cftp_admin'); ?></th>
					<th data-hide="phone"><?php _e('Қисқача маълумот','cftp_admin'); ?></th>
					<th data-hide="phone"><?php _e('Ҳужжат номи','cftp_admin'); ?></th>
					<?php
						if ($current_level != 0) {
					?>
							<th data-hide="phone"><?php _e("Ҳолати",'cftp_admin'); ?></th>
							<th data-hide="phone"><?php _e('Йўналтирилганлар','cftp_admin'); ?></th>
							<th data-hide="phone"><?php _e('Қай ҳолатда','cftp_admin'); ?></th>
					<?php
						}
					?>
					<th data-hide="phone" data-sort-ignore="true"><?php _e("Ҳаракатлар",'cftp_admin'); ?></th>
				</tr>
			</thead>
			<tbody>
			<?php
					...
					<tr>
						...
						?>
								<td class="<?php echo (!empty($uploaded['hidden'])) ? 'file_status_hidden' : 'file_status_visible'; ?>">
									...
									<?php
										$status_hidden	= __('Яширин','cftp_admin');
										$status_visible	= __('Ошкора','cftp_admin');
								...
								<td class="col_visibility">
									...
									<?php
												...
												$status_public	= __('Ошкора','cftp_admin');
												$status_private	= __('Яширин','cftp_admin');
						...
						<td>
							<a href="edit-file.php?file_id=<?php echo html_output($uploaded['new_file_id']); ?>" class="btn-primary btn btn-sm">
								<i class="fa fa-pencil"></i><span class="button_label"><?php _e('Ҳужжатни тахрирлаш','cftp_admin'); ?></span>
							</a>
							<?php
								/*
								...
								if ($current_level == 0) {
							?>
									<a href="my_files/" class="btn-primary btn btn-sm"><?php _e('Менинг ҳужжатарим','cftp_admin'); ?></a>
							<?php
								}
							?>
						</td>
...
?>
		<h3><?php _e('Ҳужжат сақланиши учун кўрилмоқда','cftp_admin'); ?></h3>
		<p><?php _e('Илтимос аудитор ҳужжат ҳақида тўлиқ маълумот киритишга харакат қилинг','cftp_admin'); ?></p>

		<?php
			if ($current_level != 0) {
		?>
			<div class="message message_info"><strong><?php _e('Ёзма','cftp_admin'); ?></strong>: <?php _e('Сиз ҳужжатни кимга йўналтиришни ўтказиб юборишингиз ҳам мумкин','cftp_admin'); ?></div>
		<?php
			}
		...
		if(!empty($empty_fields)) {
			$msg = 'Муҳим майдонларни киритинг';
...
?>
		...
		<form action="upload-process-form.php" name="save_files" id="save_files" method="post">
											...
											<div class="row edit_files_blocks">
												<div class="<?php echo ($global_level != 0 || CLIENTS_CAN_SET_EXPIRATION_DATE == '1' ) ? 'col-sm-6 col-md-3' : 'col-sm-12 col-md-12'; ?> column">
													<div class="file_data">
														<div class="row">
															<div class="col-sm-12">
																<h3><?php _e('Ҳужжат маълумотлари', 'cftp_admin');?></h3>
																...
																<div class="form-group">
																	<label><?php _e('Номи', 'cftp_admin');?></label>
																	<input type="text" name="file[<?php echo $i; ?>][name]" value="<?php echo html_output($file_title); ?>" class="form-control file_title" placeholder="<?php _e('Номини киритинг', 'cftp_admin');?>" />
																</div>
																
																<div class="form-group">
																	<label><?php _e('Ҳужжат ҳақида маълумот', 'cftp_admin');?></label>
																	<textarea name="file[<?php echo $i; ?>][description]" class="ckeditor form-control" placeholder="<?php _e('Ҳужжат маълумотларини киритинг', 'cftp_admin');?>"><?php echo (isset($description)) ? html_output($description) : ''; ?></textarea>
																</div>
													...
													<div class="col-sm-6 col-md-3 column_even column">
														<div class="file_data">
															<?php
																...
																*/
															?>
															<h3><?php _e('Ҳужжатни муддати', 'cftp_admin');?></h3>
															
															<div class="form-group">
																<label for="file[<?php echo $i; ?>][expires_date]"><?php _e('Санани танланг', 'cftp_admin');?></label>
																	<div class="input-group date-container">
																		<input type="text" class="date-field form-control datapick-field" readonly id="file[<?php echo $i; ?>][expiry_date]" name="file[<?php echo $i; ?>][expiry_date]" value="<?php echo (!empty($expiry_date)) ? $expiry_date : date('d-m-Y h:i:s'); ?>" />
															...
															<div class="checkbox">
																<label for="exp_checkbox_<?php echo $i; ?>">
																	<input type="checkbox" name="file[<?php echo $i; ?>][expires]" id="exp_checkbox_<?php echo $i; ?>" value="1" <?php if ($row['expiry_set']) { ?>checked="checked"<?php } ?> /> <?php _e('Ҳужжат муддатли', 'cftp_admin');?>
																</label>
															</div>
															...
															?>
																...
																<h3><?php _e('Барчага кўриниши мумкин', 'cftp_admin');?></h3>
																
																<div class="checkbox">
																	<label for="pub_checkbox_<?php echo $i; ?>">
																		<input type="checkbox" id="pub_checkbox_<?php echo $i; ?>" name="file[<?php echo $i; ?>][public]" value="1" /> <?php _e('Ушбу ҳужжатни барча аудиторлар кўриш имкониятига эга бўлади', 'cftp_admin');?>
																	</label>
																</div>
														<?php
												...
												?>
														<div class="col-sm-6 col-md-3 assigns column">
															<div class="file_data">
																<?php
																	...
																	*/
																?>
																<h3><?php _e('Йўналтириш', 'cftp_admin');?></h3>
																<label><?php _e('Ушбу ҳужжатни кимга йўналтирмоқчисиз ?', 'cftp_admin');?>:</label>
																<select multiple="multiple" name="file[<?php echo $i; ?>][assignments][]" class="form-control chosen-select" data-placeholder="<?php _e('Қидирув...', 'cftp_admin');?>">
																	<optgroup label="<?php _e('Аудитлар', 'cftp_admin');?>">
																		<?php
																			/**
																	...
																	<optgroup label="<?php _e('Бўлимлар', 'cftp_admin');?>">
																		<?php
																			/**
																...
																<div class="list_mass_members">
																	<a href="#" class="btn btn-xs btn-primary add-all" data-type="assigns"><?php _e('Барчаси','cftp_admin'); ?></a>
																	<a href="#" class="btn btn-xs btn-primary remove-all" data-type="assigns"><?php _e('Олиб ташлаш','cftp_admin'); ?></a>
																	<a href="#" class="btn btn-xs btn-danger copy-all" data-type="assigns"><?php _e('Бошқа ҳужжатдан нусҳалаш','cftp_admin'); ?></a>
																</div>
																...
																<div class="checkbox">
																	<label for="hid_checkbox_<?php echo $i; ?>">
																		<input type="checkbox" id="hid_checkbox_<?php echo $i; ?>" name="file[<?php echo $i; ?>][hidden]" value="1" /> <?php _e('Юкланган ҳужжат яширин ҳолатда', 'cftp_admin');?>
																	</label>
																</div>
															</div>
														</div>
														...
														<div class="col-sm-6 col-md-3 categories column">
															<div class="file_data">
																<h3><?php _e('Гуруҳ', 'cftp_admin');?></h3>
																<label><?php _e('Киритилди', 'cftp_admin');?>:</label>
																<select multiple="multiple" name="file[<?php echo $i; ?>][categories][]" class="form-control chosen-select" data-placeholder="<?php _e('Қидирув...', 'cftp_admin');?>">
																	<?php
																		/**
																...
																<div class="list_mass_members">
																	<a href="#" class="btn btn-xs btn-primary add-all" data-type="categories"><?php _e('Барчаси','cftp_admin'); ?></a>
																	<a href="#" class="btn btn-xs btn-primary remove-all" data-type="categories"><?php _e('Олиб ташлаш','cftp_admin'); ?></a>
																	<a href="#" class="btn btn-xs btn-danger copy-all" data-type="categories"><?php _e('Бошқа ҳужжатдан нусҳалаш','cftp_admin'); ?></a>
																</div>
															</div>
														</div>
													<?php
			...
			<div class="after_form_buttons">
				<button type="submit" name="submit" class="btn btn-wide btn-primary" id="upload-continue"><?php _e('Сақлаш','cftp_admin'); ?></button>
			</div>
		</form>
...
?>
		<h3><?php _e('Ҳужжат сақланмади','cftp_admin'); ?></h3>
		<table id="failed_files_tbl" class="footable" data-page-size="<?php echo FOOTABLE_PAGING_NUMBER; ?>">
			<thead>
				<tr>
					<th data-sort-initial="true"><?php _e('Ҳужжат номи','cftp_admin'); ?></th>
				</tr>
			</thead>
			<tbody>
			<?php
	...
	$(document).ready(function() {
		...
		?>
				$('.copy-all').click(function() {
					if ( confirm( "<?php _e('Бошқа ҳужжатдан нусҳаламоқчимисиз ?','cftp_admin'); ?>" ) ) {
