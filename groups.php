...
$page_title = __('Филиаллардаги бўлимларни бошқариш панели','cftp_admin');
...
if(!empty($_GET['member'])) {
	...
	if ( $sql_name->rowCount() > 0) {
		...
		while ( $row_member = $sql_name->fetch() ) {
			$page_title = ' '.__('Бўлимлар қайердалиги','cftp_admin').' '.html_entity_decode($row_member['name']).' '.__(' та','cftp_admin');
		}
		$member_exists = 1;
...
<?php
	...
	if(isset($_GET['action']) && $_GET['action'] != 'none') {
		...
		if(!empty($_GET['batch'])) {
			...
			$sql_grps->bindParam(':та бўлим', $groups_to_get);
			$sql_grps->execute();
			...
			switch($_GET['action']) {
				case 'delete':
					...
					if ($deleted_groups > 0) {
						$msg = __('Ушбу бўлим ўчирилди!','cftp_admin');
			...
			$msg = __('Энг камида битта бўлимни танланг','cftp_admin');
...
?>
	...
	<form action="groups.php" name="groups_list" method="get" class="form-inline">
		...
		<div class="form_actions_count">
			<p><?php _e('Топилди','cftp_admin'); ?>: <span><?php echo $count_for_pagination; ?> <?php _e('та бўлим','cftp_admin'); ?></span></p>
		</div>
			...
			if (!$count) {
				if (isset($no_results_error)) {
					switch ($no_results_error) {
						case 'search':
							$no_results_message = __('Хатолик, қайта уриниб кўринг!','cftp_admin');
							break;
						case 'filter':
							$no_results_message = __('Хатолик, қайта уриниб кўринг!','cftp_admin');
							break;
			...
			if ($count > 0) {
				...
				$thead_columns		= array(
											...
											array(
												...
												'sort_url'		=> 'name',
												'sort_default'	=> true,
												'content'		=> __('Бўлим номи','cftp_admin'),
											),
											array(
												'sortable'		=> true,
												'sort_url'		=> 'description',
												'content'		=> __('Қисқача маълумот','cftp_admin'),
												'hide'			=> 'phone',
											),
											array(
												'content'		=> __('Бўлим аъзолари','cftp_admin'),
											),
											array(
											...
											array(
												'sortable'		=> true,
												'sort_url'		=> 'active',
												'content'		=> __('Ошкоралиги','cftp_admin'),
											),
											array(
												'sortable'		=> true,
												'sort_url'		=> 'created_by',
												'content'		=> __('Ким тамонидан яратилинди','cftp_admin'),
												'hide'			=> 'phone',
											),
											array(
												'sortable'		=> true,
				...
				while ( $row = $sql->fetch() ) {
					 ...
					 */
					$date = date("m/d/Y h:i:s",strtotime($row['timestamp']));
					...
					$tbody_cells = array(
											...
											array(
													'actions'		=> true,
													'content'		=> '<a href="' . $files_link . '" class="btn ' . $files_btn . ' btn-sm">' . __('Кўриш','cftp_admin') . '</a>',
												),
											array(
													'actions'		=> true,
													'content'		=> '<a href="groups-edit.php?id=' . html_output( $row["id"] ) . '" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i><span class="button_label">' . __('Тахрирлаш','cftp_admin') . '</span></a>' . "\n"
												),
										);
