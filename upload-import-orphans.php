...
$page_title = __('Аудиторлар тамонидан йўналтириш учун юборилган ҳужжатлар', 'cftp_admin');
include('header.php');
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#upload_by_ftp").submit(function() {
				...
				if (checks.length == 0) { 
					alert('<?php _e('Камида биттасини танланг','cftp_admin'); ?>');
					return false; 
				} 
			});
...
$work_folder = UPLOADED_FILES_FOLDER;
		...
		if ( false === CAN_UPLOAD_ANY_FILE_TYPE ) {
			$msg = __('Сиз ушбу саҳифда аудиторлар тамонидан юкланган хужжатларни бўлим ёки гурухларга йўналтиришиннгиз мумкин', 'cftp_admin');
...
<div class="col-xs-12">
	...
	<div class="form_actions_count">
		<p class="form_count_total"><?php _e('Кўрсатиш','cftp_admin'); ?>: <span><?php echo count($files_to_add); ?> <?php _e('та ҳужжат','cftp_admin'); ?></span></p>
	</div>
		...
		<?php		
			...
			if ( isset( $files_to_add ) && count( $files_to_add ) > 0 ) {
				...
				$thead_columns		= array(
											...
											array(
												'content'		=> __('Охирги харакат','cftp_admin'),
												'hide'			=> 'phone',
												'attributes'	=> array(
																		'data-type'	=> 'numeric',
																	),
				...
				foreach ($files_to_add as $add_file) {
					...
					$tbody_cells = array(
											...
											array(
													'actions'		=> true,
													'content'		=>  '<button type="button" name="file_edit" class="btn btn-primary btn-sm btn-edit-file"><i class="fa fa-pencil"></i><span class="button_label">' . __('Тахрирлаш','cftp_admin') . '</span></button>' . "\n"
												),
										);
				...
				echo $table->render();
		?>
				<nav aria-label="<?php _e('Натижа','cftp_admin'); ?>">
		...
		<?php
			...
			/** No files found */
				...
				if (isset($no_results_error)) {
					...
					$no_results_message = __('Ҳатолик, қайта уриниб кўринг','cftp_admin');
					$no_results_message .= __('!','cftp_admin');
					$no_results_message .= '.';
				}
