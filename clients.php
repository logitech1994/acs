...
$page_title = __('Аудитларни бошқариш панели','cftp_admin');
include('header.php');
?>
		...
		if(!empty($_GET['batch'])) {
			...
			switch($_GET['action']) {
				case 'activate':
					...
					$msg = __('Танланган аудит фаол ҳолатда','cftp_admin');
				...
				case 'deactivate':
					...
					$msg = __('Танланган аудит фаол ҳолатда эмас','cftp_admin');
					echo system_message('ok',$msg);
				...
				case 'delete':
					...
					$msg = __('Танланган аудит ўчирилган','cftp_admin');
					echo system_message('ok',$msg);
			...
			$msg = __('Камида битта аудитни танланг','cftp_admin');
...
?>
		...
		<form action="clients.php" name="clients_list" method="get" class="form-inline">
							...
							<select name="action" id="action" class="txtfield form-control">
								<?php
									$actions_options = array(
															...
															'activate'		=> __('Фаол','cftp_admin'),
															'deactivate'	=> __('Фаол эмас','cftp_admin'),
			...
			<div class="form_actions_count">
				<p><?php _e('Топилди','cftp_admin'); ?>: <span><?php echo $count_for_pagination; ?> <?php _e('та аудит','cftp_admin'); ?></span></p>
			</div>
			...
			<?php
				...
				if ($count > 0) {
					...
					$thead_columns		= array(
												...
												array(
													'content'		=> __('Юклаган ҳужжатлари','cftp_admin'),
													'hide'			=> 'phone',
												),
												array(
													'content'		=> __('Ўзига тегишли ҳужжатлар','cftp_admin'),
													'hide'			=> 'phone',
												),
												array(
													'content'		=> __('Бўлимга тегишли ҳужжатлар','cftp_admin'),
													'hide'			=> 'phone',
												),
												array(
													'sortable'		=> true,
												...
												array(
													'content'		=> __('Ушбу бўлимда','cftp_admin'),
													'hide'			=> 'phone',
												),
												array(
													'content'		=> __('Хабар қилиш','cftp_admin'),
													'hide'			=> 'phone,tablet',
												),
												array(
													'sortable'		=> true,
													'sort_url'		=> 'max_file_size',
													'content'		=> __('Белгиланган ҳужжат хажми','cftp_admin'),
													'hide'			=> 'phone',
												),
												array(
													'sortable'		=> true,
													'sort_url'		=> 'timestamp',
													'content'		=> __('Қўшилган санаси','cftp_admin'),
													'hide'			=> 'phone,tablet',
												),
												/*
												array(
												...
												*/
												...
												array(
													'content'		=> __('Харакатлар','cftp_admin'),
													'hide'			=> 'phone',
												),
											);
					...
					while ( $row = $sql->fetch() ) {
						...
						$tbody_cells = array(
												...
												array(
														'content'		=> ( $row["notify"] == '1' ) ? __('Ҳа','cftp_admin') : __('Йўқ','cftp_admin'),
													),
												array(
												...
												*/
												array(
														'actions'		=> true,
														'content'		=>  '<a href="' . $files_link . '" class="btn btn-sm ' . $files_button . '">' . __("Ҳужжатлар","cftp_admin") . '</a>' . "\n" .
																			'<a href="' . $groups_link . '" class="btn btn-sm ' . $groups_button . '">' . __("Бўлимлар","cftp_admin") . '</a>' . "\n" .
																			'<a href="my_files/?client=' . html_output( $row["user"] ) . '" class="btn btn-primary btn-sm" target="_blank">' . __('Аудитдан','cftp_admin') . '</a>' . "\n"
													),
												array(
														'actions'		=> true,
														'content'		=>  '<a href="clients-edit.php?id=' . html_output( $row["id"] ) . '" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i><span class="button_label">' . __('Тахрирлаш','cftp_admin') . '</span></a>' . "\n"
													),
											);
