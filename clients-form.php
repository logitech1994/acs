...
$name_placeholder = __("ФИШ",'cftp_admin');

switch ($clients_form_type) {
	/** User is creating a new client */
	case 'new_client':
		$submit_value = __('Аудитлар қўшилди','cftp_admin');
		$disable_user = false;
		$require_pass = true;
		...
		$extra_fields = true;
		$group_field = true;
		$group_label = __('Бўлимлар','cftp_admin');
		$ignore_size = false;
		break;
	/** User is editing an existing client */
	case 'edit_client':
		$submit_value = __('Аудитлар сақланди','cftp_admin');
		$disable_user = true;
		$require_pass = false;
	...
	case 'new_client_self':
		$submit_value = __('Рўйҳатдан ўтиш','cftp_admin');
		$disable_user = false;
		$require_pass = true;
		...
		if ( CLIENTS_CAN_SELECT_GROUP == 'public' || CLIENTS_CAN_SELECT_GROUP == 'all' ) {
			$group_field = true;
			$group_label = __('Бўлимга жойлаштириш зарур','cftp_admin');
		}
		break;
	/** A client is editing his profile */
	case 'edit_client_self':
		$submit_value = __('Шаҳсий саҳифани ўзгартириш','cftp_admin');
		$disable_user = true;
		$require_pass = false;
		$form_action = 'clients-edit.php?id='.$client_id;
		$info_box = false;
...
<form action="<?php echo $form_action; ?>" name="addclient" method="post" class="form-horizontal">
	<div class="form-group">
		<label for="add_client_form_name" class="col-sm-4 control-label"><?php _e('ФИШ','cftp_admin'); ?></label>
		<div class="col-sm-8">
	...
	<div class="form-group">
		<label for="add_client_form_user" class="col-sm-4 control-label"><?php _e('Логин','cftp_admin'); ?></label>
		<div class="col-sm-8">
			<input type="text" name="add_client_form_user" id="add_client_form_user" class="form-control <?php if (!$disable_user) { echo 'required'; } ?>" maxlength="<?php echo MAX_USER_CHARS; ?>" value="<?php echo (isset($add_client_data_user)) ? html_output(stripslashes($add_client_data_user)) : ''; ?>" <?php if ($disable_user) { echo 'readonly'; }?> placeholder="<?php _e("Майдон тўлдирилиши лозим",'cftp_admin'); ?>" />
		</div>
	</div>

	<div class="form-group">
		<label for="add_client_form_pass" class="col-sm-4 control-label"><?php _e('Парол','cftp_admin'); ?></label>
		<div class="col-sm-8">
			...
			<button type="button" name="generate_password" id="generate_password" class="btn btn-default btn-sm btn_generate_password" data-ref="add_client_form_pass" data-min="<?php echo MAX_GENERATE_PASS_CHARS; ?>" data-max="<?php echo MAX_GENERATE_PASS_CHARS; ?>"><?php _e('Генерация','cftp_admin'); ?></button>
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" name="add_client_form_email" id="add_client_form_email" class="form-control required" value="<?php echo (isset($add_client_data_email)) ? html_output(stripslashes($add_client_data_email)) : ''; ?>" placeholder="<?php _e("audit_name@agrobank.uz",'cftp_admin'); ?>" />
		</div>
	</div>

	<div class="form-group">
		<label for="add_client_form_address" class="col-sm-4 control-label"><?php _e('Манзил','cftp_admin'); ?></label>
		<div class="col-sm-8">
	...
	<div class="form-group">
		<label for="add_client_form_phone" class="col-sm-4 control-label"><?php _e('Телефон номер','cftp_admin'); ?></label>
		<div class="col-sm-8">
			<input type="text" name="add_client_form_phone" id="add_client_form_phone" placeholder="<?php _e("99X XXX-XX-XX",'cftp_admin'); ?>" class="form-control" value="<?php echo (isset($add_client_data_phone)) ? html_output(stripslashes($add_client_data_phone)) : ''; ?>" />
		</div>
	</div>
	...
	?>
			<div class="form-group">
				<label for="add_client_form_intcont" class="col-sm-4 control-label"><?php _e('Лавозими','cftp_admin'); ?></label>
				<div class="col-sm-8">
			...
			<div class="form-group">
				<label for="add_client_form_maxfilesize" class="col-sm-4 control-label"><?php _e('Ҳужжат хажми','cftp_admin'); ?></label>
				<div class="col-sm-8">
					<div class="input-group">
					...
					<p class="field_note"><?php _e("Белгиланмаган ҳужжат хажми 0 mb",'cftp_admin'); ?> (Максимум: <?php echo MAX_FILESIZE; ?> mb)</p>
				</div>
			</div>
	<?php
		}
	...
	?>
				<div class="form-group assigns">
					...
					<div class="col-sm-8">
						<select multiple="multiple" name="add_client_group_request[]" id="groups-select" class="form-control chosen-select" data-placeholder="<?php _e('Майдондан қидириш ҳам мумкин', 'cftp_admin');?>">
							<?php
						...
						?>
								<div class="list_mass_members">
									<a href="#" class="btn btn-default add-all" data-type="assigns"><?php _e('Барчаси','cftp_admin'); ?></a>
									<a href="#" class="btn btn-default remove-all" data-type="assigns"><?php _e('Ўчириш','cftp_admin'); ?></a>
								</div>
						<?php
							}
						?>
			...
			<div class="form-group">
				<div class="col-sm-8 col-sm-offset-4">
					<label for="add_client_form_active">
						<input type="checkbox" name="add_client_form_active" id="add_client_form_active" <?php echo (isset($add_client_data_active) && $add_client_data_active == 1) ? 'checked="checked"' : ''; ?>> <?php _e('Фаол (аудит тизимга кирса)','cftp_admin'); ?>
					</label>
				</div>
			</div>
	<?php
	...
	<div class="form-group">
		<div class="col-sm-8 col-sm-offset-4">
			<label for="add_client_form_notify_upload">
				<input type="checkbox" name="add_client_form_notify_upload" id="add_client_form_notify_upload" <?php echo (isset($add_client_data_notify_upload) && $add_client_data_notify_upload == 1) ? 'checked="checked"' : ''; ?>> <?php _e('E-mail га юкланганлик ҳақида ҳабар кетиш','cftp_admin'); ?>
			</label>
		</div>
	</div>
			...
			<!-- <div class="form-group">
				<div class="col-sm-8 col-sm-offset-4">
					<label for="add_client_form_notify_account">
						<input type="checkbox" name="add_client_form_notify_account" id="add_client_form_notify_account" <?php echo (isset($add_client_data_notify_account) && $add_client_data_notify_account == 1) ? 'checked="checked"' : ''; ?>> <?php _e('E-mail га ҳуш келибсиз','cftp_admin'); ?>
					</label>
				</div>
			</div> -->
	<?php
	...
	?>
				<div class="form-group">
					<label class="col-sm-4 control-label"><?php _e('Текшириш','cftp_admin'); ?></label>
					<div class="col-sm-8">
	...
	<?php
		if ($info_box == true) {
			$msg = __('Сиз юборган маълумотни E-mail орқали тасдиқлаб олишингиз мумкин','cftp_admin');
