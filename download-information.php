...
$page_title = __('Сақлаш маълумотномаси','cftp_admin');
...
<div class="col-xs-12">
	<?php	
		if ($page_status === 0) {
			$msg = __('Ҳужжат танланмаган','cftp_admin');
			echo system_message('error',$msg);
			echo '<p>'.$direct_access_error.'</p>';
		}
		else if ($page_status === 2) {
			$msg = __('Ҳужжатнитанланг','cftp_admin');
			echo system_message('error',$msg);
			echo '<p>'.$direct_access_error.'</p>';
		}
		else {
	?>
		...
		<form action="download-information.php" name="groups_list" method="get" class="form-inline">
			...
			<div class="row">
				<div class="col-sm-12">
					<h3><?php _e('Юкланишлар сони','cftp_admin'); ?>: <span class="label label-primary"><strong><?php echo $file_stats['total']; ?></strong></span></h3>
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12">
					...
					<?php
						...
						*/
						...
						$thead_columns		= array(
													...
													array(
														'content'		=> __('Вақти','cftp_admin'),
													),
													array(
														'sortable'		=> true,
														'sort_url'		=> 'user_id',
														'content'		=> __('Аудит','cftp_admin'),
													),
													array(
														'sortable'		=> true,
														'sort_url'		=> 'anonymous',
														'content'		=> __('Анонимлар','cftp_admin'),
													),
													array(
														'sortable'		=> true,
														'sort_url'		=> 'remote_ip',
														'content'		=> __("Аудит IP манзили",'cftp_admin'),
														'hide'			=> 'phone',
													),
													array(
														'sortable'		=> true,
														'sort_url'		=> 'remote_host',
														'content'		=> __("Аудит ҳостноми",'cftp_admin'),
														'hide'			=> 'phone',
													),
												);
						...
						$tfoot_columns		= array(
													...
													array(
														'content'		=> __('киритилинмаган фойдаланувчилар / аудитлар','cftp_admin') . ': <span class="label label-primary">' . $file_stats['unique_clients'] . '</span>',
													),
													array(
														'content'		=> __('Умумий юкланишлар сони','cftp_admin') . ': <span class="label label-primary">' . $file_stats['anonymous_users'] . '</span>',
													),
													array(
														'content'		=> '',
													),
						...
						while ( $row = $sql->fetch() ) {
							 ...
							 */
							$anon_yes	= __('Ҳа','cftp_admin');
							$anon_no	= __('Йўқ','cftp_admin');
