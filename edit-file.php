...
$page_title = __('Ҳужжатни тахрирлаш','cftp_admin');
include('header.php');
	...
	<?php
		...
		if (isset($no_results_error)) {
			switch ($no_results_error) {
				case 'no_id_passed':
					$no_results_message = __('Ушбу ҳужжатни тахрирлаш учун сиз, гуруҳга масъул аудитга бириктиришингиз керак','cftp_admin');
					break;
				case 'id_not_exists':
					...
					if(!empty($file['name'])) {
						...
						*/
						...
						if($process_file['database'] == true) {
							...
							$msg = __('Ушбу ҳужжат тахрирланди','cftp_admin');
			...
			<form action="edit-file.php?file_id=<?php echo filter_var($this_file_id,FILTER_VALIDATE_INT); ?>" method="post" name="edit_file" id="edit_file">
				<div class="container-fluid">
										...
										<div class="row edit_files_blocks">
													...
													<div class="row">
														<div class="col-sm-12">
															...
															<div class="form-group">
																...
																<input type="text" name="file[<?php echo $i; ?>][name]" value="<?php echo html_output($row['filename']); ?>" class="form-control file_title" placeholder="<?php _e('Ҳужжатга уникал ном берилиши лозим', 'cftp_admin');?>" />
															</div>
	
															<div class="form-group">
																<label><?php _e('Тўлиқ маълумот', 'cftp_admin');?></label>
																<textarea name="file[<?php echo $i; ?>][description]" class="ckeditor form-control" placeholder="<?php _e('Ҳужжат ҳақида қисқача маълумот киритишингиз мумкин', 'cftp_admin');?>"><?php echo (!empty($row['description'])) ? html_output($row['description']) : ''; ?></textarea>
															</div>
														</div>
													</div>
												</div>
														...
														<div class="file_data">
															...
															<div class="checkbox">
																<label for="exp_checkbox">
																	<input type="checkbox" id="exp_checkbox" name="file[<?php echo $i; ?>][expires]" value="1" <?php if ($row['expires']) { ?>checked="checked"<?php } ?> /> <?php _e('Ҳужжат муддати', 'cftp_admin');?>
																</label>
															</div>
															...
															<h3><?php _e('Ошкора сақлаш', 'cftp_admin');?></h3>
															<div class="checkbox">
																<label for="pub_checkbox">
																	<input type="checkbox" id="pub_checkbox" name="file[<?php echo $i; ?>][public]" value="1" <?php if ($row['public_allow']) { ?>checked="checked"<?php } ?> /> <?php _e('Ушбу ҳужжатни барча кўриши имконияти мавжуд', 'cftp_admin');?>
																</label>
															</div>

															<div class="divider"></div>
															<h3><?php _e('Ошкора URL', 'cftp_admin');?></h3>
											...
											?>
													<div class="col-sm-6 col-md-3 assigns column">
														<div class="file_data">
															...
															<select multiple="multiple" name="file[<?php echo $i; ?>][assignments][]" class="form-control chosen-select" data-placeholder="<?php _e('Қидирув...', 'cftp_admin');?>">
																<optgroup label="<?php _e('Аудиторлар', 'cftp_admin');?>">
																	<?php
																		/**
															...
															<div class="checkbox">
																<label for="hid_checkbox">
																	<input type="checkbox" id="hid_checkbox" name="file[<?php echo $i; ?>][hidden]" value="1" /> <?php _e('Аудитор ва тизим фойдаланувчиларга йўналтирилган барча ҳужжатлар ҳолати ҳақида E-mail га жщнатилинмайди', 'cftp_admin');?>
																</label>
															</div>
															<div class="checkbox">
																<label for="hid_existing_checkbox">
																	<input type="checkbox" id="hid_existing_checkbox" name="file[<?php echo $i; ?>][hideall]" value="1" /> <?php _e('Аудитор ва тизим фойдаланувчиларга йўналтирилган барча ҳужжатлар яширин ҳолатда', 'cftp_admin');?>
																</label>
															</div>
														</div>
													</div>
													...
													<div class="col-sm-6 col-md-3 categories column">
														<div class="file_data">
															<h3><?php _e('Гуруҳлар', 'cftp_admin');?></h3>
															<label><?php _e('Киритиш', 'cftp_admin');?>:</label>
					...
					<div class="after_form_buttons">
						<a href="<?php echo BASE_URI; ?>manage-files.php" name="cancel" class="btn btn-default btn-wide"><?php _e('Бекор қилиш','cftp_admin'); ?></a>
						<button type="submit" name="submit" class="btn btn-wide btn-primary"><?php _e('Сақлаш','cftp_admin'); ?></button>
					</div>
				</div>
			</form>
	<?php
