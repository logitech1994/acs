...
switch ($user_form_type) {
	case 'new_user':
		$submit_value = __('Фойдаланувчи қўшиш','cftp_admin');
		$disable_user = false;
		$require_pass = true;
		...
		break;
	case 'edit_user':
		$submit_value = __('Фойдаланувчини сақлаш','cftp_admin');
		$disable_user = true;
		$require_pass = false;
		...
		break;
	case 'edit_user_self':
		$submit_value = __('Фойдаланувчини тахрирлаш','cftp_admin');
		$disable_user = true;
		$require_pass = false;
		$form_action = 'users-edit.php?id='.$user_id;
		$extra_fields = false;
...
<form action="<?php echo html_output($form_action); ?>" name="adduser" method="post" class="form-horizontal">
	<div class="form-group">
		<label for="add_user_form_name" class="col-sm-4 control-label"><?php _e('ФИШ','cftp_admin'); ?></label>
		<div class="col-sm-8">
	...
	<div class="form-group">
		<label for="add_user_form_user" class="col-sm-4 control-label"><?php _e('Логин','cftp_admin'); ?></label>
		<div class="col-sm-8">
			<input type="text" name="add_user_form_user" id="add_user_form_user" class="form-control <?php if (!$disable_user) { echo 'required'; } ?>" maxlength="<?php echo MAX_USER_CHARS; ?>" value="<?php echo (isset($add_user_data_user)) ? html_output(stripslashes($add_user_data_user)) : ''; ?>" <?php if ($disable_user) { echo 'readonly'; } ?> placeholder="<?php _e("Тўлдирилиши лозим майдон",'cftp_admin'); ?>" />
		</div>
	</div>

	<div class="form-group">
		<label for="add_user_form_pass" class="col-sm-4 control-label"><?php _e('Парол','cftp_admin'); ?></label>
		<div class="col-sm-8">
			...
			<button type="button" name="generate_password" id="generate_password" class="btn btn-default btn-sm btn_generate_password" data-ref="add_user_form_pass" data-min="<?php echo MAX_GENERATE_PASS_CHARS; ?>" data-max="<?php echo MAX_GENERATE_PASS_CHARS; ?>"><?php _e('Генерация','cftp_admin'); ?></button>
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" name="add_user_form_email" id="add_user_form_email" class="form-control required" value="<?php echo (isset($add_user_data_email)) ? html_output(stripslashes($add_user_data_email)) : ''; ?>" placeholder="<?php _e("username@agrobank.uz",'cftp_admin'); ?>" />
		</div>
	</div>
		...
		?>
			<div class="form-group">
				<label for="add_user_form_level" class="col-sm-4 control-label"><?php _e('Мақоми','cftp_admin'); ?></label>
				<div class="col-sm-8">
			...
			<div class="form-group">
				<label for="add_user_form_maxfilesize" class="col-sm-4 control-label"><?php _e('Максимум ҳужжат хажми','cftp_admin'); ?></label>
				<div class="col-sm-8">
					<div class="input-group">
					...
					<p class="field_note"><?php _e("Лимит белгилаш лозим",'cftp_admin'); ?> (Максимум: <?php echo MAX_FILESIZE; ?> mb)</p>
				</div>
			</div>
			...
			<div class="form-group">
				<div class="col-sm-8 col-sm-offset-4">
					<label for="add_user_form_active">
						<input type="checkbox" name="add_user_form_active" id="add_user_form_active" <?php echo (isset($add_user_data_active) && $add_user_data_active == 1) ? 'checked="checked"' : ''; ?> /> <?php _e('Фаол (фойдаланувчи тизимга кирса)','cftp_admin'); ?>
					</label>
				</div>
			</div>
					...
					<div class="form-group">
						<div class="col-sm-8 col-sm-offset-4">
							<label for="add_user_form_notify_account">
								<input type="checkbox" name="add_user_form_notify_account" id="add_user_form_notify_account" <?php echo (isset($add_user_data_notify_account) && $add_user_data_notify_account == 1) ? 'checked="checked"' : ''; ?> /> <?php _e('E-mail га ҳабар жўнатиш','cftp_admin'); ?>
							</label>
						</div>
					</div>
			<?php
	...
	<?php
		if ($user_form_type == 'new_user') {
			$msg = __('E-mail орқали тасдиқлашингиз мумкин','cftp_admin');
